...
const FIREBASE_PATH = "/QAT2 Output.json";
...
// NOTE: previously this list held Team Leader staff IDs and the query filtered
// on `team_leader_staff_id`. It now holds plain `staff_id` values and the query
// filters directly on `staff_id`, so you can pass ANY staff member's ID here
// or via the `staff_ids` / `staff_id` query params at request time.
const TEAM_LEADERS = [
  "G26658-OTL",
  "G25883-OTL",
  "G22371-OTL",
...
| BUILD SQL
|--------------------------------------------------------------------------
| Filters directly on `staff_id` now (instead of `team_leader_staff_id`),
| so any individual staff member's ID can be searched, not just team leaders.
|--------------------------------------------------------------------------
*/
function buildQuery(staffId) {
  // basic sanitization - staff IDs are alphanumeric / dash based in this system
...
async function saveToFirebase(allData) {
  ...
  console.log(`  Firebase updated: ${allData.length} staff records`);
}
...
/*
|--------------------------------------------------------------------------
| FETCH SINGLE STAFF MEMBER (by staff_id) with Denominator
|--------------------------------------------------------------------------
*/
async function fetchSingleStaff(staffId) {
  console.log(`  Fetching: staff_id="${staffId}"`);
  const query = buildQuery(staffId);
...
/*
|--------------------------------------------------------------------------
| FETCH MULTIPLE STAFF MEMBERS (by staff_id list)
|--------------------------------------------------------------------------
*/
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // Default staff id list (renamed conceptually from "team-leaders")
